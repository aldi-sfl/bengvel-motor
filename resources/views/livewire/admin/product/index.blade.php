<div>
    ...
    <script>
        document.addEventListener('livewire:load', function() {
            ...
            document.querySelectorAll('[data-carousel="slide"]').forEach(carousel => new Carousel(carousel));
        });
...
<div class="owl-carousel owl-theme">
    ...
    <div class="item-">
        <div class="card">
            <img src="{{ Storage::url($image->image_url) }}"  alt="{{ $item->name }}">
        </div>
    </div>
    @endforeach
...
@endforeach   --}}
    ...
    @if ($selectedProduct)
        @livewire('admin.product.preview', ['selectedProduct' => $selectedProduct], key('product-detail-' . $selectedProduct->id))
    @elseif ($updateSelectedProduct)
        @livewire('admin.product.edit', ['updateSelectedProduct' => $updateSelectedProduct], key('product-detail-' . $updateSelectedProduct->id))
    @else
     {{-- old table product --}}
...
{{-- ================================================================== --}}
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                    <div class="w-full md:w-1/2">
                        ...
                        <form class="flex items-center">
                            ...
                            <div class="relative w-full">
                                ...
                                <input type="text" id="simple-search" wire:model.debounce.300ms="searchTerm"
                    ...
                    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <button type="button" wire:click="showModalAdd()" class="flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="px-2 py-4">Product name</th>
                            ...
                            <tr class="border-b dark:border-gray-700">
                                <td class="p-4">
                                    @if($item->images->first())
                                        <img class="w-16 md:w-32 max-w-full max-h-full" src="{{ asset('storage/' . $item->images->first()->image_url) }}" alt="image not found" title="{{ $item->name }}">
                                    @endif
                                </td>
                                <td class="px-2 py-3 font-medium text-bold text-gray-900 whitespace-nowrap dark:text-white">{{ $item->name }}</td>
                                <td class="px-4 py-3">{{ $item->price }}</td>
                                <td class="px-4 py-3">{{ $item->stock }}</td>
                                <td class="px-2 py-3">{{ $item->category->category_name }}</td>
                                <td class="px-4 py-3 max-w-[12rem] truncate">{{ $item->description}}</td>
                                
                                {{-- action --}}
                                <td class="px-4 py-3 flex items-center justify-end ">
                                    <button wire:click="showDropdown({{ $item->id }})" id="dropdownButton-{{ $item->id }}" class="dropdown-button inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                        @if($dropdownId === $item->id)
                                            <!-- 'X' icon when dropdown is open -->
                                            <svg class="w-5 h-5" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M10 8.586l4.293-4.293a1 1 0 011.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 111.414-1.414L10 8.586z" clip-rule="evenodd"></path>
                                            </svg>
                                        @else
                                            <!-- Dropdown dots icon when dropdown is closed -->
                                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                            </svg>
                                        @endif
                                    </button>
                                    <div  id="dropdown-{{ $item->id }}" class="{{ $dropdownId === $item->id ? '' : 'hidden' }} dropdown-menu z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm" aria-labelledby="dropdownButton-{{ $item->id }}">
                                            <li>
                                                <button type="button" wire:click="updateProduct({{ $item->id }})" {{-- data-modal-target="updateProductModal" data-modal-toggle="updateProductModal" --}} class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                                    ...
                                                    <div wire:loading wire:target="updateProduct">
                                                        <svg aria-hidden="true" class="content-end w-5 h-5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/><path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/></svg>
                                                        <span class="sr-only">Loading...</span>
                                                    </div>
                                                </button>
                                            </li>
                                            <li>
                                                <button type="button" wire:click="getProduct({{ $item->id }})" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                                    <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                    ...
                                                    Preview
                                                    &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 
                                                    <div wire:loading wire:target="getProduct">
                                                        <svg aria-hidden="true" class="content-end w-5 h-5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/><path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/></svg>
                                                        <span class="sr-only">Loading...</span>
                                                    </div>
                                                </button>
                                            ...
                                            <li>
                                                <button type="button" wire:click="showDeleteModal({{ $item->id }})" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
                                                    ...
                                                    Delete
                                                    &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 
                                                    <div wire:loading wire:target="showDeleteModal">
                                                        <svg aria-hidden="true" class="content-end w-5 h-5 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/><path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/></svg>
                                                        <span class="sr-only">Loading...</span>
                                                    </div>
                                                </button>
    ...
    <!-- Create modal -->
    <div  tabindex="-1" aria-hidden="true" class="{{ $showModalAdd ? '' : 'hidden' }} bg-black bg-opacity-50 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center flex items-center w-full md:inset-0 h-[calc(100%-1rem)] h-full">
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                    ...
                    <button type="button" wire:click="closeModalAdd" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-target="createProductModal" data-modal-toggle="createProductModal">
                ...
                <!-- Modal body -->
                    ...
                    <form wire:submit.prevent="store" wire:poll.500ms enctype="multipart/form-data">
                        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                            <div class="sm:col-span-2">
                                ...
                                @error('name') <p class="mt-2 text-xs text-red-600 dark:text-red-400"><span class="font-medium">Error.</span> {{ $message }}</p> @enderror
                            </div>
                            <div class="w-full">
                                ...
                                <input wire:model="image" id="imageInput" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" type="file" multiple>
                                @push('scripts')
                                <script>
                                ...
                                @error('image') <p class="mt-2 text-xs text-red-600 dark:text-red-400"><span class="font-medium">Error.</span> {{ $message }}</p>  @enderror
                            </div>
                            <div class="w-full">
                            ...
                            <div>
                                ...
                                @error('category_id') <p class="mt-2 text-xs text-red-600 dark:text-red-400"><span class="font-medium">Error.</span> {{ $message }}</p>  @enderror
                            </div> 
                            <div class="sm:col-span-2">
                                ...
                                @error('description') <p class="mt-2 text-xs text-red-600 dark:text-red-400"><span class="font-medium">Error.</span> {{ $message }}</p>  @enderror
                        ...
                        &nbsp; &nbsp;
                        &nbsp;                        
                        <button type="button" wire:click="closeModalAdd" class="inline-flex items-center px-5 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Close</button>
                    </form>
    ...
    <div id="updateProductModal" tabindex="-1" aria-hidden="true" class="hidden bg-black bg-opacity-50 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] h-full">
        <div class="relative p-4 w-full max-w-2xl max-h-full">
            <!-- Modal content -->
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                <!-- Modal header -->
                <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Update Products</h3>
    ...
    <!-- Delete modal -->
    @if($showDeleteModal)
    <div id="popup-modal" tabindex="-1" class="bg-black bg-opacity-50 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 flex justify-center items-center h-full w-full">
        <div class="relative p-4 w-full max-w-md">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <!-- Close Button -->
                <button wire:click="closeDeleteModal" type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 dark:hover:bg-gray-600 dark:hover:text-white">
                ...
                <div class="py-6 px-6 lg:px-8 text-center">
                    ...
                    <button wire:click="deleteProduct" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2">Yes, I'm sure</button>
                    <button wire:click="closeDeleteModal" class="text-gray-900 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 dark:focus:ring-gray-600 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:text-white font-medium rounded-lg text-sm px-5 py-2.5 ml-2 mb-2 dark:bg-gray-800 dark:text-gray-400">No, cancel</button>
                </div>
            </div>
        </div>
    </div>
