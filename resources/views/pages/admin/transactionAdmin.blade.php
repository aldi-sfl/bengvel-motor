...
<section class="bg-white py-2 antialiased dark:bg-gray-900 md:py-16">
  <h2 class="text-4xl font-bold dark:text-white flex flex-col justify-center items-center pb-6">Transaction List</h2>
    <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
      <div class="mx-auto max-w-5xl">

        <div class="gap-4 sm:flex sm:items-center sm:justify-between">
          {{-- <h2 class="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">Transaction list</h2> --}}
          <div class="flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-white dark:bg-gray-900">
                <form method="GET" action="{{ route('transaction') }}" class="flex items-center space-x-4">
                    <div class="relative">
                        ...
                        <input type="text" name="search" id="table-search-users" class="block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for transactions by name" value="{{ request('search') }}">
                    </div>
                    <button type="submit" class="px-4 py-2 sr-only text-white bg-blue-600 rounded">Search</button>
                </form>
            </div>
          ...
          <div class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <div class="flex flex-wrap items-center gap-y-4 py-6">
              ...
              <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                ...
                <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">{{ $transaction->created_at->format('d M Y') }}</dd>
              </dl>
  
              <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                <dt class="text-base font-medium text-gray-500 dark:text-gray-400">Buyer</dt>
                <dd class="mt-1.5 text-base font-semibold text-gray-900 dark:text-white">{{ $transaction->user->name }}</dd>
              </dl>
  
              <dl class="w-1/2 sm:w-1/4 lg:w-auto lg:flex-1">
                
                <form class="" action="{{ route('transaction.changeStatus', $transaction->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <select id="status" name="transaction_status" class="block w-full p-2 mb-6 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    ...
                    <div class="w-full grid sm:grid-cols-2 lg:flex lg:w-64 lg:items-center lg:justify-end gap-4">
                        <button type="submit" class="w-full rounded-lg bg-primary-700 px-3 py-2 text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 lg:w-auto">save status</button>
                    
                        <a href="{{ route('transaction-detail', $transaction->id) }}" class="w-full inline-flex justify-center rounded-lg  border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700 lg:w-auto">View details</a>
                    </div>
                </form>
            </div>
            @endforeach
